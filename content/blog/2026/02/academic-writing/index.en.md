...
- Zotero is easy to use and has all the features that make a great reference manager system:
  ...
  - it can retrieve documents metadata from identifiers (ISBN, DOI, PubMed ID, or arXiv ID, see ['Adding Items to Zotero'](https://www.zotero.org/support/adding_items_to_zotero)) and directly from PDF metadata,
  ...
  - it supports a large array of citation formats (available as [CSL files](https://www.zotero.org/support/dev/citation_styles)).
...
The only Mendeley feature I have missed so far is the search in body text of the entire collection of PDF (very useful for non-native English speakers). That said, as all PDF files are actually stored in one place, I simply created an alias using the [`grep`](https://help.ubuntu.com/community/grep) command line: 

```sh
$ alias szot='grep -rnw ~/Zotero/storage/ -e'
```

which fulfills my need! This command should work on most Unix-like
operating systems and I am pretty sure there are simple ways to do this on Windows. 
...
1. [WYSIWYG](https://en.wikipedia.org/wiki/WYSIWYG) ("What You See Is What You Get") editors, represented by the popular word processors such as LibreOffice, Google Docs, and Microsoft Word (and there are Zotero plugins for these three options)
...
This is the raw way of doing the conversion, using the default (and super light) Pandoc's HTML and PDF templates. Note that Pandoc has tens of options (flags) and [filters](https://pandoc.org/filters.html) to further customize documents. For instance, there are filters to cross-reference sections, figures, tables and equations (see [pandoc-xnos](https://github.com/tomduck/pandoc-xnos)). Furthermore there are higher level tools leveraging Pandoc that make the rendering easier with better-looking (and heavier) templates. For instance, in the R ecosystem, R users could use R Markdown and something like this to produce both documents at once: 
